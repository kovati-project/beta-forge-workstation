...
Starts ComfyUI, Real-ESRGAN, and Rembg, then installs ComfyUI Manager inside the container. InvokeAI is not started (it's behind the `studio` profile).
...
Minimum required for image generation.
...
```bash
# Accept license at https://huggingface.co/black-forest-labs/FLUX.1-dev first
...
Open `http://10.10.10.2:8188` after deployment.
...
Then confirm in the browser (`http://10.10.10.2:8188`):
- Checkpoint visible in model list (refresh if needed)
- Queue a simple txt2img workflow, confirm output appears in `/data/outputs/comfyui/`
- `nvidia-smi` shows GPU0 VRAM at 8–12GB during generation
...
# Start InvokeAI (stop ComfyUI first — both use GPU0)
ssh kasemo@10.10.10.2 "docker stop comfyui && \
  docker compose -f ~/ai-workstation/docker/compose.studio.yml --profile studio up -d invokeai"
...
| Symptom | Check |
|---------|-------|
| Checkpoint not in model list | Confirm `.safetensors` exists in `/data/models/comfyui/checkpoints/`; restart container |
| ComfyUI Manager missing after update | Re-run `deploy-phase04.sh` — it re-installs if not found |
| FLUX.1 OOM | Confirm no other service holds GPU0 VRAM: `nvidia-smi` |
| Real-ESRGAN not at `:8189` | `docker logs realesrgan` — image may still be pulling on first start |
| Rembg not at `:8190` | `docker logs rembg` — CPU-only service, should start quickly |
